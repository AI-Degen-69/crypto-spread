...
- `server/osc_dash.py` — FastAPI dashboard on `:8802`. Pages: `/`, `/oscillation`, `/summary`, `/analysis`. APIs: `/api/oscillation`, `/api/goals`, `/api/analysis`, `/api/backtest` (query: offset/queue/pair_cost/exit_*), `/api/ticks/manifest|verify|file|upload-chunk|upload-stream`, collector control `/api/collector/status|start|stop|poll-once`, live execution & cockpit `/api/live/account|state|control|orders|cancel_all|cancel_order|test_order|config|latency|stream`.
