...
- **RTDS Crypto Feed & WebSockets**: `prices.crypto.binance` supports `btcusdt`, `ethusdt`, `solusdt`, `xrpusdt` only (no BNB on Binance topic; use Chainlink or REST). CLOB Market WebSocket (`wss://ws-subscriptions-clob.polymarket.com/ws/market`) supports multi-token subscription on a single socket with 10s PING heartbeat and `custom_feature_enabled` for `best_bid_ask`. Use 1s RTDS ticks as leading price signals for faster stop-loss execution.
