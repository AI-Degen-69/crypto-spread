...
- `tests/` — targeted gates for live-trader and strategy parity work: `test_engine_parity.py` (canonical behavioral parity gate holding both engines to `docs/engine-decision-rules.md`; any change to engine decision logic must add or update a parity scenario in the same PR), `test_live_trader.py`, `test_backtest_engine.py`, `test_fresh_start_parity.py`, `test_osc_dash_integration.py`. Run targeted test files during development and review (<2s).
