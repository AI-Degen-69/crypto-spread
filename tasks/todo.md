...
- [x] **T1** `[Backend/Logic]` Backtest: delete both gates + re-entry, add per-tick `quote_range`
- [x] **T2** `[Backend/Logic]` Live: delete both gates + re-entry, add per-tick `quote_range`
- [x] **T3** `[Test/Parity]` `tests/test_quote_range_parity.py` — entry, exit, re-entry to the range, both engines
