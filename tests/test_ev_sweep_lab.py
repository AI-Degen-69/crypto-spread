...
def test_sim2_rejects_knobs_no_research_simulator_implements(kwargs):
    """Issue #164 put knobs on `BacktestParams` that `sim2` never reads.

    Measured before the guard: flipping any of them left `sim2`'s output
    identical across 400 real windows — exactly the defect #182 fixed for
    `entry_delay_sec`/`entry_band`, reopened by #164. Issue #229 replaced the
    deleted stop-loss/timeout knobs with the dead-zone trio, which the engine
    applies and no research simulator implements; the guard moves with it.
    """
    from backtest.engine import BacktestParams
...
def test_sim2_itself_enforces_the_guard():
    ...
    w = _one_tick_window()
    with pytest.raises(ValueError, match="sim2 ignores"):
        sim2(w, BacktestParams(dead_zone_val=0.20))
...
def test_the_guard_compares_against_defaults_not_truthiness():
    """`if getattr(p, k)` — the shape the guard had — would wave through any
    knob whose changed value is falsy, e.g. `dead_zone_val=0.0`.

    The comparison is against the declared default, so the disable value and
    the default are distinguished even when both are falsy.
    """
    from backtest.engine import BacktestParams
    # The default itself never trips the guard, on any knob.
    ev_lab.reject_knobs_sim2_ignores(BacktestParams())
    ev_lab._reject_unsupported_knobs(BacktestParams())
    # A non-default value — even a falsy one — does.
    with pytest.raises(ValueError):
        ev_lab.reject_knobs_sim2_ignores(BacktestParams(dead_zone_val=0.0))
